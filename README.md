...
However, the flood of notifications emails produced when using GitHub (one for every comment on every issue thread) can seem overwhelming. Below are some tips for how to follow repos effectively.
...
Sometimes, after trying to `git push`, you get an error message. You should read this carefully: most of the time it is because the remote repo you are pushing to has changed, and you just need to pull, and fix any conflicts, before you push.
